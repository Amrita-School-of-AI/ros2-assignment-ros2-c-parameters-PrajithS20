...
public:
  ...
  : Node("param_node")
  {
    ...
    // 2. Create a timer that fires every 2000ms (2 seconds)
    timer_ = this->create_wall_timer(
      2000ms, std::bind(&ParamNode::timer_callback, this));
      
    RCLCPP_INFO(this->get_logger(), "ParamNode has been started.");
  }

private:
  void timer_callback()
  {
    // 3. Read the current values of the parameters
    std::string robot_name = this->get_parameter("robot_name").as_string();
    double max_speed = this->get_parameter("max_speed").as_double();
    bool enabled = this->get_parameter("enabled").as_bool();

    // 4. Log the values to the terminal
